    ...
    while i + seq_len <= data_len:
        ...
        for idx in range(bsz_per_host):
            ...
            feature = {
                ...
                "is_masked": _int64_feature(1 * is_masked),
...
def _local_perm(inputs, targets, is_masked, perm_size, seq_len):
    ...
    non_mask_tokens = tf.logical_and(
        tf.logical_not(is_masked), non_func_tokens)
...
def get_input_fn(
    ...
    for idx, record_dir in enumerate(tfrecord_dirs):
        #record_glob = 'record_info-train-0-0.bsz-32.seqlen-512.reuse-256.uncased.bi.alpha-6.beta-1.fnp-85.json'
        ...
        record_paths = ['./data_small/tfrecords/record_info-train-0-0.bsz-' + str(bsz_per_host) + '.seqlen-' + str(seq_len) + \
        '.reuse-' + str(reuse_len) + '.uncased.bi.alpha-6.beta-1.fnp-85.json']
        #print(record_paths)
        #exit()
        ...
        for record_info_path in record_paths:
            if num_passes is not None:
                ...
                if len(fields) == 5 and pass_id >= num_passes:
                    tf.logging.info("Skip pass %d: %s",
                                    pass_id, record_info_name)
                    continue
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string("sp_path", "./spiece/spiece.model",
